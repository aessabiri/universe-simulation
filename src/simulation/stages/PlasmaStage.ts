...
export class PlasmaStage extends Stage {
  ...
  init() {
    ...
    for (let i = 0; i < this.particleCount; i++) {
      const i3 = i * 3;
      const r = 2 + Math.random() * 18; // Keep center clear, spread wide
      ...
      if (rand < 0.4) color.setHex(0xaa00ff); 
      else if (rand < 0.8) color.setHex(0x00ffff); 
      else color.setHex(0xff0088); 
  ...
  update(time: number, delta: number) {
    ...
    for (let i = 0; i < this.particleCount; i++) {
      ...
      const speed = 1.5;
      posAttr[i3] = x + Math.sin(t * speed + y) * 3.0;
      posAttr[i3+1] = y + Math.cos(t * speed + z) * 3.0;
      posAttr[i3+2] = z + Math.sin(t * speed + x) * 3.0;
    }
    this.particles.geometry.attributes.position.needsUpdate = true;

    if (this.filaments && Math.random() > 0.85) {
        ...
        for (let i = 0; i < 800; i++) {
            ...
            filAttr[i6+3] = posAttr[pIdx] + (Math.random()-0.5)*3.0;
            filAttr[i6+4] = posAttr[pIdx+1] + (Math.random()-0.5)*3.0;
            filAttr[i6+5] = posAttr[pIdx+2] + (Math.random()-0.5)*3.0;
        }
