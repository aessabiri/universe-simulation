...
export class GalaxyStage extends Stage {
  ...
  init() {
    ...
    const colorInside = new THREE.Color('#ffcc88'); // Warm core stars
    const colorOutside = new THREE.Color('#88aaff'); // Cooler arm stars
    ...
    for (let i = 0; i < starCount; i++) {
      ...
      starCol[i3] = mixedColor.r;
      starCol[i3 + 1] = mixedColor.g;
      starCol[i3 + 2] = mixedColor.b;
      starSiz[i] = Math.random() * 2;
    }
    starGeo.setAttribute('position', new THREE.BufferAttribute(starPos, 3));
    starGeo.setAttribute('color', new THREE.BufferAttribute(starCol, 3));
    this.stars = new THREE.Points(starGeo, new THREE.PointsMaterial({
      size: 0.08, vertexColors: true, map: TextureUtils.createCircularParticleTexture(),
      transparent: true, alphaTest: 0.01, blending: THREE.AdditiveBlending, depthWrite: false
    }));
    this.galaxyGroup.add(this.stars);

    // 2. GALACTIC CORE (Bulge)
    ...
    for (let i = 0; i < coreCount; i++) {
      ...
      corePos[i3 + 1] = r * Math.sin(phi) * Math.sin(theta) * 0.6; // Slightly flattened
    ...
    this.core = new THREE.Points(coreGeo, new THREE.PointsMaterial({
      size: 0.12, vertexColors: true, map: TextureUtils.createCircularParticleTexture(),
      transparent: true, opacity: 0.6, blending: THREE.AdditiveBlending, depthWrite: false
    }));
    this.galaxyGroup.add(this.core);

    // 3. HII REGIONS & STAR CLUSTERS (Pink/Blue arm pockets)
    ...
    // 4. DUST LANES (Dark obscuring particles)
    ...
    for (let i = 0; i < dustCount; i++) {
      ...
      const spinAngle = radius * parameters.spin + 0.3; // Offset from stars
      const branchAngle = ((i % parameters.branches) / parameters.branches) * Math.PI * 2;
  ...
  private createDistantGalaxies() {
    ...
    for (let i = 0; i < distantCount; i++) {
      ...
      // Mini spiral galaxy (simplified for distance)
      ...
      for (let j = 0; j < miniCount; j++) {
        ...
        const angle = r * 1.5 + (j % 2) * Math.PI; // Simple 2-arm spiral
      ...
      const miniPoints = new THREE.Points(miniGeo, new THREE.PointsMaterial({
        size: 0.15, vertexColors: true, map: TextureUtils.createCircularParticleTexture(),
        transparent: true, opacity: 0.6, blending: THREE.AdditiveBlending, depthWrite: false
      }));
